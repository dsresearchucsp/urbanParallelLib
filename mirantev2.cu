...
__global__ void mapKernel(
    ...
    if (idx < crime_count) {
        ...
        for (int j = 0; j < edge_count; j++) {
            // Coordenadas del edge (x1,y1,x2,y2)
            float x1 = edges[j * 4];
...
private:
    ...
    std::unordered_map<std::string, std::pair<float, float>> node_coordinates; // ID -> (lon, lat)
...
public:
    ...
    void loadEdgesWithNodes(const std::string &filename) {
        edges.clear();
        ...
        // Saltar header: Node1_ID,Node1_Latitude,Node1_Longitude,Node2_ID,Node2_Latitude,Node2_Longitude
        std::getline(file, line);
        node_coordinates.clear();
        
        while (std::getline(file, line)) {
            std::stringstream ss(line);
            std::string node1_id, node1_lat_str, node1_lon_str;
            std::string node2_id, node2_lat_str, node2_lon_str;
            
            if (std::getline(ss, node1_id, ',') && 
                std::getline(ss, node1_lat_str, ',') && 
                std::getline(ss, node1_lon_str, ',') && 
                std::getline(ss, node2_id, ',') && 
                std::getline(ss, node2_lat_str, ',') && 
                std::getline(ss, node2_lon_str)) {
                try {
                    float node1_lat = std::stof(node1_lat_str);
                    float node1_lon = std::stof(node1_lon_str);
                    float node2_lat = std::stof(node2_lat_str);
                    float node2_lon = std::stof(node2_lon_str);
                    
                    // Guardar coordenadas de nodos con sus IDs
                    node_coordinates[node1_id] = {node1_lon, node1_lat};
                    node_coordinates[node2_id] = {node2_lon, node2_lat};
                    
                    // Almacenar edge como x1,y1,x2,y2 (lon,lat,lon,lat)
                    edges.push_back(node1_lon);
                    edges.push_back(node1_lat);
                    edges.push_back(node2_lon);
                    edges.push_back(node2_lat);
                    
                    // std::cout << "Edge: " << node1_id << " (" << node1_lon << "," << node1_lat 
                    //          << ") to " << node2_id << " (" << node2_lon << "," << node2_lat << ")" << std::endl;
                } catch (const std::exception& e) {
                    std::cerr << "Error parsing: " << line << std::endl;
                }
            }
        ...
        std::cout << "Loaded " << node_coordinates.size() << " unique nodes" << std::endl;
    }
    
    std::string findNodeIdByCoordinates(float x, float y) {
        // Buscar el nodo con las coordenadas exactas
        const float epsilon = 1e-6f;
        
        for (const auto& pair : node_coordinates) {
            const std::string& node_id = pair.first;
            float node_x = pair.second.first;
            float node_y = pair.second.second;
            
            if (fabsf(node_x - x) < epsilon && fabsf(node_y - y) < epsilon) {
                return node_id;
            }
        }
        
        // Si no se encuentra exacto, devolver coordenadas como string
    ...
    void processMapReduce() {
        ...
        // Escribir archivo CSV con formato: NodeID, Latitude, Longitude, Weight
        std::ofstream output_file("nodes_with_crimes.csv");
        output_file << "NodeID,Latitude,Longitude,Weight\n";
        
        for (const auto& result : h_results) {
            // Encontrar el ID del nodo basado en las coordenadas
            std::string node_id = findNodeIdByCoordinates(result.key_x, result.key_y);
            
            output_file << node_id << ","         // NodeID
                       << result.key_y << ","     // Latitude
                       << result.key_x << ","     // Longitude
                       << result.value << "\n";   // Weight
        }
        ...
        std::cout << "Archivo generado: nodes_with_crimes.csv" << std::endl;
        std::cout << "Formato: NodeID, Latitude, Longitude, Weight" << std::endl;
        
        // Mostrar los 10 nodos con más crímenes
        ...
        for (int i = 0; i < std::min(10, (int)h_results.size()); i++) {
            std::string node_id = findNodeIdByCoordinates(h_results[i].key_x, h_results[i].key_y);
            std::cout << i+1 << ". " << node_id 
                     << " (" << h_results[i].key_y << "," << h_results[i].key_x << "): " 
                     << h_results[i].value << " crímenes" << std::endl;
        }
    }
    
    int run() {
        try {
            loadCrimeSpots("crime_locations.csv");
            loadEdgesWithNodes("edges_with_nodes.csv");  // Nuevo archivo
            processMapReduce();
        } catch (const std::exception &e) {
